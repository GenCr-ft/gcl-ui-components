...
status: complete
issue-id: GenCr-ft/gcl-ui-components#54
metadata:
...
# WI-54 — `CLAUDE.md` shim

## Problem

Claude Code reads `CLAUDE.md`, not `AGENTS.md` (<https://code.claude.com/docs/en/memory> § AGENTS.md), so
`gcl-ui-components` had no Claude-readable project context — while the workspace `CLAUDE.md` designates each repo's
